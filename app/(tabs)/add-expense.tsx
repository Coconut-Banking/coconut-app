    ...
    <SafeAreaView style={s.root}>
      ...
      <Modal visible={showSplitDetail} transparent animationType="slide" onRequestClose={() => setShowSplitDetail(false)}>
        <Pressable style={s.sheetOverlay} onPress={() => setShowSplitDetail(false)}>
          <Pressable style={[s.sheetCard, { maxHeight: "80%" }]} onPress={(e) => e.stopPropagation()}>
            <View style={s.sheetHandle} />
            <View style={s.pickerHead}>
            ...
            <ScrollView>
                    ...
                    <View key={p.key} style={[s.splitDetailRow, i < splitPeople.length - 1 && s.listRowBorder]}>
                      ...
                      <View style={{ flex: 1 }}>
                        ...
                        <Text style={s.listRowSub}>
                          ...
                          {splitMethod === "percent" && `${customSplits[p.key] || "0"}%`}
                        </Text>
                      </View>
              ...
              <View style={s.remainingRow}>
                <Text style={s.remainingLabel}>
                  {splitMethod === "shares" ? "Total" : "Remaining"}
                </Text>
                <Text style={s.remainingValue}>
                  {splitMethod === "shares" && `${splitPeople.reduce((a, p) => a + (parseFloat(customSplits[p.key] || "1") || 1), 0)} shares`}
                  {splitMethod === "exact" && `$${total.toFixed(2)}`}
                  {splitMethod === "percent" && "100%"}
                </Text>
              </View>
            </ScrollView>
            <View style={s.footer}>
