  ...
  const connectTapToPay = useCallback(async () => {
    ...
    try {
      ...
      if (!locRes.ok) {
        ...
        throw new Error((errData as { error?: string }).error ?? `Location request failed (${locRes.status})`);
      }
  ...
  const collectPayment = useCallback(async () => {
    ...
    try {
      ...
      if (!piRes.ok) {
        ...
        throw new Error((errData as { error?: string }).error ?? `Payment intent creation failed (${piRes.status})`);
      }
