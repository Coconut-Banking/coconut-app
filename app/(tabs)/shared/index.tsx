...
function FriendsPage({ friends, w, refreshControl, onAddFriend }: { friends: FriendBalance[]; w: number; refreshControl?: React.ReactElement<import("react-native").RefreshControlProps>; onAddFriend?: () => void }) {
  return (
    <ScrollView style={{ width: w }} contentContainerStyle={st.page} showsVerticalScrollIndicator={false} refreshControl={refreshControl}>
      {!friends.length ? (
        <View style={st.empty}>
          ...
          {onAddFriend && (
            <TouchableOpacity style={st.emptyBtn} onPress={onAddFriend}>
              <Ionicons name="person-add" size={16} color="#fff" />
              <Text style={st.emptyBtnText}>Add friend</Text>
            </TouchableOpacity>
          )}
        </View>
      ) : (
        <>
          ...
          {onAddFriend && (
            <TouchableOpacity style={st.addRow} onPress={onAddFriend} activeOpacity={0.7}>
              <View style={st.addIcon}><Ionicons name="person-add" size={16} color="#9CA3AF" /></View>
              <Text style={st.addText}>Add friend</Text>
            </TouchableOpacity>
          )}
        </>
      )}
...
function GroupsPage({ groups, w, onCreate, refreshControl }: { groups: GroupSummaryType[]; w: number; onCreate: () => void; refreshControl?: React.ReactElement<import("react-native").RefreshControlProps> }) {
  return (
    <ScrollView style={{ width: w }} contentContainerStyle={st.page} showsVerticalScrollIndicator={false} refreshControl={refreshControl}>
      {!groups.length ? (
        <View style={st.empty}>
...
function ActivityPage({ items, w, refreshControl }: { items: RecentActivityItem[]; w: number; refreshControl?: React.ReactElement<import("react-native").RefreshControlProps> }) {
  return (
    <ScrollView style={{ width: w }} contentContainerStyle={st.page} showsVerticalScrollIndicator={false} refreshControl={refreshControl}>
      {!items.length ? (
        <View style={st.empty}>
...
export default function SharedIndex() {
  ...
  const addFriend = async () => {
    ...
    if (__DEV__) console.log("[addFriend] name:", JSON.stringify(name), "email:", JSON.stringify(email));
    ...
    try {
      ...
      if (!memberRes.ok) {
        // Clean up the orphan group since member add failed
        apiFetch(`/api/groups/${group.id}`, { method: "DELETE" }).catch(() => {});
        Alert.alert("Error", memberData?.error ?? "Failed to add friend");
        return;
      }
      // Close form first for instant feedback, then refetch in background
  ...
  const renderPage = useCallback(({ item }: { item: number }) => {
    switch (item) {
      case 0: return <FriendsPage friends={summary?.friends ?? []} w={width} refreshControl={refreshControl} onAddFriend={() => setShowAddFriend(true)} />;
      ...
      <View style={st.pad}>
        ...
        {showAddFriend && (
          <View style={st.createCard}>
            ...
            />
            <TextInput
              style={[st.createInput, { marginTop: 8 }]}
              value={friendEmail}
            ...
            <View style={{ flexDirection: "row", gap: 8, marginTop: 8 }}>
              <TouchableOpacity style={st.createBtn} onPress={addFriend} disabled={!friendName.trim() || addingFriend} activeOpacity={0.7}>
                <Text style={st.createBtnText}>{addingFriend ? "…" : "Add"}</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={() => { setShowAddFriend(false); setFriendName(""); setFriendEmail(""); }} activeOpacity={0.7}>
                <Text style={st.cancelText}>Cancel</Text>
              </TouchableOpacity>
            </View>
          </View>
